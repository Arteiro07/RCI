...
void close_all_exit(infos *info);
...
void connect_s(int argc, char *argv[], infos *info)
{
	...
	for (i = 1; i < argc; i+=2)
	{
		...
		else if(strcmp("-u", argv[i]) == 0)
		{
			...
			if(bind(info->fd_c,(struct sockaddr*)&addr, sizeof(addr)) == -1)
			{
				perror("Bind UDP_C");
				close_all_exit(info);
			}
		...
		else if(strcmp("-t", argv[i]) == 0)
		{
			...
			addr.sin_addr.s_addr = htonl(INADDR_ANY);
			addr.sin_port = htons((u_short)aux);
			if(bind(info->fd_s_wait,(struct sockaddr*)&addr, sizeof(addr)) == -1)
			{
				perror("Bind TCP_wait");
				close_all_exit(info);
			}

			if(listen(info->fd_s_wait, 5) == -1)
			{
				perror("Listen TCP_wait");
				close_all_exit(info);
			}
		}
...
void manage_fd(infos *info, states *state)
{
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if(state->cs != -1)
				send_message_cs(info, state, 0, true);
		}
...
void process_commands_cs(infos *info, states *state, char msg[])
{
	...
	else if(state->cs == WITHDRAW_DS)
	{
		...
		if(state->available == false)
		{
			...
			if(sendto(info->fd_c, str, strlen(str)+1, 0,(struct sockaddr*)&(info->addr_c),addrlen) == -1)
			{
				perror("sendto CS");
				close_all_exit(info);
			}
			if(debug)printf("SENT_C: %s\n", msg);
	...
	else if(state->cs == WITHDRAW_START)
	{
		...
		else if(state->alone == false && state->leave == true)
		{
			send_message_s(info, TOKEN, 'O', info->id_next, inet_ntoa(info->addr_s_next.sin_addr), ntohs(info->addr_s_next.sin_port));
		}
		else if(state->leave)
...
void send_message_cs(infos *info, states *state, int mode, bool repeat)
{
	...
	if(mode == SET_DS || (repeat && mode_ == SET_DS))
	{
		...
		if(sendto(info->fd_cs, msg, strlen(msg)+1, 0,(struct sockaddr*)&(info->addr_cs), addrlen) == -1)
		{
			perror("sendto UDP_CS");
			close_all_exit(info);
		}
	}
